...
pub unsafe fn genTangSpace<I: MikktGeometry>(geometry: &mut I, fAngularThreshold: f32) -> bool {
    ...
    let fThresCos: f32 =
        ((fAngularThreshold * std::f32::consts::PI / 180.0f32) as f64).cos() as f32;
    f = 0;
    while f < iNrFaces {
...
unsafe fn DegenEpilogue<I: MikktGeometry>(
...
unsafe fn GenerateTSpaces<I: MikktGeometry>(
...
unsafe fn EvalTspace<I: MikktGeometry>(
...
unsafe fn InitTriInfo<I: MikktGeometry>(
    mut pTriInfos: *mut STriInfo,
    mut piTriListIn: *const i32,
    geometry: &mut I,
...
unsafe fn CalcTexArea<I: MikktGeometry>(geometry: &mut I, mut indices: *const i32) -> f32 {
...
unsafe fn GenerateSharedVerticesIndexList<I: MikktGeometry>(
...
unsafe fn MergeVertsFast<I: MikktGeometry>(
    mut piTriList_in_and_out: *mut i32,
...
unsafe fn GenerateInitialVerticesIndexList<I: MikktGeometry>(
    ...
    while f < geometry.num_faces() {
        ...
        if !(verts != 3 && verts != 4) {
            ...
            if verts == 3 {
                ...
                let distSQ_02: f32 = (T2 - T0).mag2();
                let distSQ_13: f32 = (T3 - T1).mag2();
                ...
                if distSQ_02 < distSQ_13 {
                    ...
                    let distSQ_02_0: f32 = (P2 - P0).mag2();
                    let distSQ_13_0: f32 = (P3 - P1).mag2();
