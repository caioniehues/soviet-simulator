...
One Cargo package: a tiny `src/main.rs` calling into `src/lib.rs` (which builds the `App`, sets the primary window, adds `DefaultPlugins`, and registers `SimPlugins` + `game::GamePlugins`), feature plugins under `src/game/`, and `assets/` for runtime-loaded files only (keep refs/captures out of it). Include dev-profile opt settings so iteration isn't glacial:
...
- `SimPlugins` and `GamePlugins` (`src/lib.rs`, `src/game/mod.rs`) are `PluginGroup`s, not a list a binary retypes. Build from the group and state what a scenario *excludes* — `SimPlugins.build().disable::<HeatSimPlugin>()` — never what it includes: HUD/tool systems hard-require sim resources (`Res<T>` panics on a missing resource at runtime, not compile time), so a binary that hand-types a subset silently rots the moment a rung adds a plugin the subset never learns about. A binary that adds `GamePlugins` needs the full `SimPlugins` group underneath it, undisabled, for the same reason.
