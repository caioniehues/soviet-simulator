...
|---|---|---|---|---|---|
| **No teleportation** | Stock changes hands only at a physical endpoint by a physical carrier; never at match, payment or reservation | charter pillar 1 | logistics, trade, water (§004), production | `scenario_0082_dispatch_gates_stock_not_match` (`simulation/src/tests/scenarios/hoarding.rs:138`), `sov_abs_ext_trade_import_is_physical` (`simulation/src/tests/scenarios/ledger.rs:595`), `sov_uo5_border_custody_conserved_across_import` and `sov_uo5_empty_border_stock_import_waits_in_tosource` (`simulation/src/tests/scenarios/sov_uo5_border_stock.rs:140,245`), `sov_7f7_import_money_settles_at_loading_not_at_match` and `sov_7f7_export_money_not_moved_at_match` (`simulation/src/tests/scenarios/sov_7f7_settlement.rs:114,227`), plus the `PillarLedger` conservation sweep inside every `sentinel_journey` test — GUARDED for domestic timing, import border custody, and border-money timing. Exports and imports both push a dispatch at match and debit/credit only at physical arrivals (seller `market.rs:1597`, border `:1801`, buyer `:1795`). | — (the 2026-09-03 export match-time seller debit was removed by `sov-7f7`/`sov-20g`; no known violation remains) |
