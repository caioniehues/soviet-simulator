...
| `EVID-WASTE-005` | `cargo test -p simulation evid_waste_container_generation_disposition_single_authority -- --test-threads=1` — Waste alone creates and changes each typed `WasteGenerationID`, `WasteContainerID`, capacity/fill, collection request, overflow/backlog, processing disposition, and landfill-retention record; a container fill remains a physical quantity and each transition has one Waste-owned audit record. | Let Buildings, Healthcare, Logistics, Resources, or Production mutate container type/capacity/fill, create a collection request or overflow/backlog record, choose/overwrite a disposition or landfill retention, duplicate a Waste transition record, or replace physical fill with a cleanliness score. | Inspected typed generation, container fill/capacity, collection/overflow timeline, disposition, landfill-retention ledger, and Waste transition audit. |
