...
| `EVID-TRADE-006` | `cargo test -p simulation evid_trade_domestic_operations_never_clear_roubles -- --test-threads=1` | `SPEC-TRADE-001`: domestic matching, allocation, reservation, dispatch, production, and consumption leave rouble balances unchanged and do not rank work by price; only completed physical customs clearance settles the fixed border amount. | Debit/credit roubles or rank a domestic operation by price, or settle before customs clearance. | Inspected domestic transition ledger and subsequent customs-clearance capture. |
| `EVID-TRADE-007` | `cargo test -p simulation evid_trade_medicine_cargo_and_water_transport_tags -- --test-threads=1` | `SPEC-TRADE-005`: Medicine import retains request, allocation, reservation, pickup, custody, delivery, and external consumption distinctions; Water has no freight haul and clears only from its completed metered utility-transfer result. | Consume Medicine at allocation, omit its custody transition, attach a freight haul to Water, or clear Water without its completed transfer ID. | Inspected Medicine haul/custody timeline and Water transfer-tagged customs capture. |
