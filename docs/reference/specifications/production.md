...
| `EVID-PRODUCTION-006` | `cargo test -p simulation evid_production_non_price_domestic_run -- --test-threads=1` | `SPEC-PRODUCTION-004`: equal physically delivered recipe inputs produce the same domestic run result when enterprise rouble balances or quoted prices differ; no production transition mutates roubles. | Debit domestic roubles for a run, reject a physically feasible run for lack of domestic money, or rank input consumption by price. | Inspected delivered inputs, run result, and unchanged domestic-rouble ledger. |
