...
| `EVID-CITIZENS-006` | `cargo test -p simulation evid_citizens_activity_intent_references_single_authority -- --test-threads=1` — for one Citizen ID, Citizens alone records current intended activity and ID references to home, household, work, education, health, and itinerary outcomes; each referenced module alone changes its corresponding assignment, capacity, need, treatment, or route outcome, and Citizen retains only the resulting reference. | Let Citizens set a work/education/health capacity, mutate a Household membership or residence, satisfy a Needs outcome, record Healthcare treatment, overwrite a Pathfinding/Logistics itinerary outcome, or let a referenced module create a second citizen activity/assignment copy. | Inspected the Citizen ID's activity-intent timeline, all six references, and the owning-module transition/result audit. |
