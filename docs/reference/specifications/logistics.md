...
| `EVID-LOGISTICS-007` | `cargo test -p simulation evid_logistics_single_fulfillment_authority -- --test-threads=1` | `SPEC-LOGISTICS-001`: one haul authority references the durable demand and external consumption ID, records each allocation-to-release transition, and neither owns consumed state nor mutates consumed quantity. | Add a second fulfillment owner, copy consumption into the haul, or let the haul consume delivered stock. | Inspected haul timeline with demand, consumption, custody, and authority identifiers. |
| `EVID-LOGISTICS-008` | `cargo test -p simulation evid_logistics_non_price_domestic_dispatch -- --test-threads=1` | `SPEC-LOGISTICS-005`: identical physical candidates produce the same domestic dispatch order when rouble balances or quoted prices differ; border settlement is absent until separate customs clearance. | Rank a domestic haul by price or roubles, or settle roubles during allocation, reservation, or dispatch. | Inspected candidate inputs, dispatch order, and separate customs-state capture. |
| `EVID-LOGISTICS-009` | `cargo test -p simulation evid_logistics_roads_owned_parking_recovery -- --test-threads=1` | `SPEC-LOGISTICS-007`: Logistics requests recovery for the actual vehicle location; Roads exclusively grants or refuses one parking reservation, and the vehicle remains unparked while acknowledgement is pending. | Let Logistics create/overwrite a parking reservation, park without Roads acknowledgement, or relocate the vehicle to a slot. | Inspected recovery request, Roads verdict, slot holder, and vehicle location. |
