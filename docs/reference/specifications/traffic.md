...
| `EVID-TRAFFIC-005` | `cargo test -p simulation evid_traffic_planner_pressure_cause_distinction -- --test-threads=1` — `SPEC-TRAFFIC-004`: the Planner-visible record distinguishes capacity pressure/queue/stall and their contributing lane, vehicle/job, and blocking cause from a route failure or unrelated input shortage. | Collapse causes into one generic shortage, hide the contributor/blocking reason, or report a route failure as traffic pressure. | Inspected side-by-side bottleneck, route-failure, and input-shortage captures. |
| `EVID-TRAFFIC-006` | `cargo test -p simulation evid_traffic_reports_freight_without_clearing -- --test-threads=1` — `SPEC-TRAFFIC-005`: traffic pressure can report delay/stall to the referenced Logistics haul but cannot satisfy a need, clear a domestic request, transfer cargo, or mutate roubles. | Mark a need/request complete from Traffic, transfer freight from a stall, or settle/debit roubles in a traffic update. | Inspected traffic event, unchanged need/request/custody/rouble ledgers, and linked haul status. |
| `EVID-TRAFFIC-007` | `cargo test -p simulation evid_traffic_observes_roads_parking_without_mutation -- --test-threads=1` — `SPEC-TRAFFIC-006`: Traffic observes a Roads-owned parking reservation as blocked/unblocked; only Roads changes the slot holder, while Vehicles reference it and Logistics recovery waits on Roads. | Let Traffic reserve/release a slot, alter its holder during blockage observation, or let Logistics recover before Roads responds. | Inspected parking ownership, blockage observation, and recovery acknowledgement timeline. |
