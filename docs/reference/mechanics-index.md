...
| Export-side physical clearance | trade | 1.0 — charter row Transport and border | [logistics](../simulation/physical-economy/logistics.md) | trade | **CONTRADICTED** (money clears at match for both halves: `simulation/src/economy/mod.rs:104` applies `money_delta` before `advance_dispatches`; seller debited at match, `simulation/src/economy/market.rs:732-741`) | E-121 |
| Import as physical truck | trade | 1.0 — charter row Transport and border | [logistics](../simulation/physical-economy/logistics.md) | trade, logistics | PARTIAL (no border source stock — `sov-uo5`: the seller is a stockless `FreightStation`, `simulation/src/souls/freight_station.rs:31-36`; buyer stock waits for the truck but `money_delta` clears at match, `simulation/src/economy/mod.rs:104`) | E-128 |
...
| Custody conservation on cancel/return | logistics | 1.0 — charter row Transport and border | [custody](../simulation/physical-economy/custody.md) | logistics | PARTIAL (bounded Loading/Returning route failure deletes already-debited cargo with no declared sink — `sov-bub`, `simulation/src/economy/market.rs:950-976`) | E-136 |
...
| Durable unmet demand (queue with age) | economy | 1.0 — charter row Households and citizens | [queues (concept)](../simulation/concepts/queues.md) | needs, production | **CONTRADICTED** (non-human unmatched orders extracted on the fallback path, `simulation/src/economy/market.rs:629-632`) | ECO-SUB-001 |
