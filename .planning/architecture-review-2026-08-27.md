...
| id | finding | strength |
|---|---|---|
| lua-validation | **Probe-proven**: recipe amount 0 → validator passes, 20+ sim tests panic (divide-by-zero, money.rs:193); `get_lua().unwrap_or()` swallows TYPE errors at 5 sites (corrected from 6, 2026-08-28: `base.rs:17`, `item.rs:24`, `recipe.rs:63`, `zone.rs:20`, `zone.rs:21`; correct form exists 2 lines away); multiplier −3 → 4,294,967,293 standing request accepted as ordinary demand (proven end-to-end); ="not-a-number" → silently honest company (deletes the core loop). One fix: range checks in `validate()` + switch those 5 sites to `get_lua_opt` + delete the `dbg!` at market.rs:1124 (fires per Market::default). Second zero source at market.rs:1118. | **S** |
