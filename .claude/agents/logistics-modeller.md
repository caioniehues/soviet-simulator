...
`truck = Some(v)` has no timeout and its only exit is the vehicle entity vanishing — that is
the wedge shape, and it has now produced four tickets. And: a refusal signal is only safe
